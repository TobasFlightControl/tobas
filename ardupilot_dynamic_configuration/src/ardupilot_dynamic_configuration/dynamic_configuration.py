...
class DynamicConfigurationWidget(MainWidget):
    ...
    def __init__(self) -> None:
        self._pkg_name = osp.dirname(__file__)
        super().__init__(self._pkg_name)

        icon_path = osp.join(
            rospkg.RosPack().get_path(self._pkg_name), "resources/icon.png"
        )
    ...
    def _on_save_button_clicked(self) -> None:
        ...
        try:
            with open(file_path, "w") as f:
                # Write header
                f.write(
                    f"# This file was automatically generated by {self._pkg_name} on {datetime.now()}\n"
                )
                f.write(f"#\n")
                ...
                for p in self._params.values():
                    f.write(
                        f'{p["vehicle_id"]}\t{p["component_id"]}\t{p["name"]}\t{p["value"]}\t{p["type"]}\n'
                    )
