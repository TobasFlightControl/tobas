...
#include "../include/tobas_calibration_core/measure_sensor_noise.hpp"
...
void MeasureSensorNoise::run()
{
  ...
  for (size_t i = 0; i < kDataCount; ++i)
  {
    ...
    usleep(tobas_real::kWaitToRefreshBarometer);

    // モータが動いている状態でのノイズを計測するため，PWMコマンドを送信
    constexpr double max_pwm_period =
      tobas_real::kPwmMin + (tobas_real::kPwmMax - tobas_real::kPwmMin) * kMaxThrottle;
    ...
    if (pres_data(i) < tobas_real::kMinAirPressure || tobas_real::kMaxAirPressure < pres_data(i))
  ...
  pt.put(tobas_real::kConfigKey_AccNoiseDensity, acc_noise_density);
  pt.put(tobas_real::kConfigKey_GyroNoiseDensity, gyro_noise_density);
  pt.put(tobas_real::kConfigKey_MagNoiseDensity, mag_noise_density);
  pt.put(tobas_real::kConfigKey_PressureNoiseDensity, pres_noise_density);
  pt.save();
  cout << "The result is saved to '" << tobas_real::kConfigPath << "'." << endl;
...
void MeasureSensorNoise::sendDisarm()
{
  ...
  {
    setPeriodOnAllChannels(tobas_real::kPwmDisarm);
    usleep(tobas_real::kDisarmInterval * 1000000);
  }
}
...
void MeasureSensorNoise::accelerateMotors()
{
  ...
  while (elapsed_time < kPwmUpDownTime)
  {
    ...
    const auto pwm_period =
      tobas_real::kPwmMin + (tobas_real::kPwmMax - tobas_real::kPwmMin) * throttle;
