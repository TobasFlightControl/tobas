...
{
PositionYawPublisher::PositionYawPublisher(rclcpp::Node::SharedPtr node, rclcpp::Node::SharedPtr pnh, const string& name)
  : super(node, pnh, name)
{
...
void PositionYawPublisher::run()
{
  ...
  RCLCPP_INFO_STREAM("Waiting for '" << tobas::kTakeoffAction << "' action server.");
  if (!takeoff.waitForServer(rclcpp::Duration(kWaitForExternalActionServer)))
  {
    RCLCPP_ERROR_STREAM("Failed to connect to '" << tobas::kTakeoffAction << "' action server.");
    return;
  }
  ...
  {
    RCLCPP_ERROR_STREAM("'" << tobas::kTakeoffAction << "' action failed: " << takeoff_state.getText());
    return;
  }
  RCLCPP_INFO_STREAM("Takeoff finished successfully.");
  ...
  {
    RCLCPP_ERROR_STREAM("Failed to get " << node_.getNamespace() << "/" << tobas::kOdometryTopic << ".");
    cmd_pos_.x() = 0;
    cmd_pos_.y() = 0;
  ...
  while (node_.ok())
  {
    ...
    switch (c)
    {
      case 'w':  // X+
      {
        ...
        RCLCPP_INFO_STREAM("[Moving forward] pos[m]: " << cmd_pos_ << ", yaw[rad]: " << cmd_yaw_);
        break;
      }
      case 's':  // X-
      {
        cmd_pos_.x(x_limit_.clamp(cmd_pos_.x() - delta_pos_));
        RCLCPP_INFO_STREAM("[Moving backward] pos[m]: " << cmd_pos_ << ", yaw[rad]: " << cmd_yaw_);
        break;
      }
      case 'a':  // Y+
      {
        cmd_pos_.y(y_limit_.clamp(cmd_pos_.y() + delta_pos_));
        RCLCPP_INFO_STREAM("[Moving left] pos[m]: " << cmd_pos_ << ", yaw[rad]: " << cmd_yaw_);
        break;
      }
      case 'd':  // Y-
      {
        cmd_pos_.y(y_limit_.clamp(cmd_pos_.y() - delta_pos_));
        RCLCPP_INFO_STREAM("[Moving right] pos[m]: " << cmd_pos_ << ", yaw[rad]: " << cmd_yaw_);
        break;
      }
      case keyboard::UP:  // Z+
      {
        cmd_pos_.z(z_limit_.clamp(cmd_pos_.z() + delta_pos_));
        RCLCPP_INFO_STREAM("[Moving up] pos[m]: " << cmd_pos_ << ", yaw[rad]: " << cmd_yaw_);
        break;
      }
      case keyboard::DOWN:  // Z-
      {
        cmd_pos_.z(z_limit_.clamp(cmd_pos_.z() - delta_pos_));
        RCLCPP_INFO_STREAM("[Moving down] pos[m]: " << cmd_pos_ << ", yaw[rad]: " << cmd_yaw_);
        break;
      }
      case keyboard::LEFT:  // Yaw+
      {
        cmd_yaw_ = yaw_limit_.clamp(cmd_yaw_ + delta_rot_);
        RCLCPP_INFO_STREAM("[Rotating left] pos[m]: " << cmd_pos_ << ", yaw[rad]: " << cmd_yaw_);
        break;
      }
      case keyboard::RIGHT:  // Yaw-
      {
        cmd_yaw_ = yaw_limit_.clamp(cmd_yaw_ - delta_rot_);
        RCLCPP_INFO_STREAM("[Rotating right] pos[m]: " << cmd_pos_ << ", yaw[rad]: " << cmd_yaw_);
        break;
      }
    }
...
void PositionYawPublisher::getRosParams()
{
  ...
  ros2::getParam(pnh_, "pose_limit/x/min", x_limit_.lower, kDefaultMinimumX);
  ros2::getParam(pnh_, "pose_limit/x/max", x_limit_.upper, kDefaultMaximumX);
  ros2::getParam(pnh_, "pose_limit/y/min", y_limit_.lower, kDefaultMinimumY);
  ros2::getParam(pnh_, "pose_limit/y/max", y_limit_.upper, kDefaultMaximumY);
  ros2::getParam(pnh_, "pose_limit/z/min", z_limit_.lower, kDefaultMinimumZ);
  ros2::getParam(pnh_, "pose_limit/z/max", z_limit_.upper, kDefaultMaximumZ);
  ros2::getParam(pnh_, "pose_limit/yaw/min", yaw_limit_.lower, kDefaultMinimumYaw);
  ros2::getParam(pnh_, "pose_limit/yaw/max", yaw_limit_.upper, kDefaultMaximumYaw);

  TOBAS_CHECK(x_limit_.isValid());
  TOBAS_CHECK(y_limit_.isValid());
  TOBAS_CHECK(z_limit_.isValid());
  TOBAS_CHECK(yaw_limit_.isValid());
}
}  // namespace tobas_keyboard_teleop
