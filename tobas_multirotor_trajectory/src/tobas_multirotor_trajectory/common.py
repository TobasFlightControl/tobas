...
class FollowTrajectoryClient(ABC):
    ...
    def run(self) -> None:
        ...
        if not self._ac.wait_for_server(Duration.from_sec(self.WAIT_FOR_SERVER)):
            self.get_logger().error(f'Failed to connect to "{self.ACTION_NAME}" action server')
            return
    ...
    def _call_action_and_show_result(self, goal: FollowPositionYawTrajectoryGoal) -> None:
        self.get_logger().info(f'Sending a goal to "{self.ACTION_NAME}" action.')
        self._ac.send_goal_and_wait(goal)
        rclpy.logdebug("Action finished.")
    ...
    def _on_shutdown(self) -> None:
        self.get_logger().info("Program interrupted before completion. Canceling action.")
