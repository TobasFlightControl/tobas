...
private:
  ...
  dsp::NoiseVarianceFilter<double, 3, kNoiseFilterWindowSize> mag_noise_;
...
void MagPreprocessNode::magRawCb(const tobas_msgs::MagneticFieldStamped::ConstSharedPtr& mag_raw)
{
  // Initialize
  if (!mag_raw_) {
    if (!mag_noise_.initialize(kNoiseFiltrerHpfCutoff, mag_raw->mag.data)) {
      TOBAS_ERROR("Failed to initialize magnetic field noise variance filter.");
      return;
    }
    mag_raw_ = mag_raw;
    return;
  ...
  // Update noise filter
  mag_noise_.update(mag_raw->mag.data, dt);
