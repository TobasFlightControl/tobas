...
void SourceTableReader::read(const std::string& data)
{
  std::stringstream data_stream(data);
  std::string moint_point_data;
  while(std::getline(data_stream, moint_point_data)) {
    if (moint_point_data.substr(0, 3) == "STR") { // SOURCETABLE 200 OKなどのserver以外のデータを表す行は保存しない．serverのデータなら最初はSTRとなっているはず．
      ...
      if (splitted_size!= kBITRATE + 1 || splitted_size != kMISC + 1){ // データの個数がbit rateまでのサイズか，miscまでのサイズの場合は推定正しい
...
std::string SourceTableReader::findNearestMountPoint(const double& latitude, const double& longitude)
{
  size_t min_idx = 0;
  ...
  for (size_t i = 0; i < moint_points_.size(); i++) {
    ...
    auto moint_point_x = std::cos(latitude_of_moint_point * kDEG_TO_RAD) * std::cos(longitude_of_moint_point * kDEG_TO_RAD);
    auto moint_point_y = std::cos(latitude_of_moint_point * kDEG_TO_RAD) * std::sin(longitude_of_moint_point * kDEG_TO_RAD);
    ...
    double distance_squared = std::pow((moint_point_x - query_point_x), 2) + std::pow((moint_point_y - query_point_y), 2) + std::pow((moint_point_z - query_point_z), 2);
    if (min_distance_squared > distance_squared) {
      min_distance_squared = distance_squared;
      min_idx = i;
    }
  }
  return moint_points_[min_idx][kMOUNT_POINT];
}
