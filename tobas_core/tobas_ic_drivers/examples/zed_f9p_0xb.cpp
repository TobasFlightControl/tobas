...
#include <tobas_ic_drivers/ublox/zed_f9p_0xb.hpp>
...
int main(int argc, char** argv)
{
  ...
  cout << "Configuring baudrate." << endl;
  if (!gnss.configureBaudRate(115200)) {
    cerr << "Failed to configure baudrate." << endl;
    return EXIT_FAILURE;
  }
  ...
  if (!gnss.enableMsg(ublox::ZEDF9P0xB::CLASS_NAV, ublox::ZEDF9P0xB::NAV_COV, true)) {
    cerr << "Failed to enable NAV_COV message." << endl;
    return EXIT_FAILURE;
  }
  if (!gnss.enableMsg(ublox::ZEDF9P0xB::CLASS_NAV, ublox::ZEDF9P0xB::NAV_HPPOSLLH, true)) {
    cerr << "Failed to enable NAV_HPPOSLLH message." << endl;
    return EXIT_FAILURE;
  }
  if (!gnss.enableMsg(ublox::ZEDF9P0xB::CLASS_NAV, ublox::ZEDF9P0xB::NAV_POSLLH, true)) {
    cerr << "Failed to enable NAV_POSLLH message." << endl;
    return EXIT_FAILURE;
  }
  if (!gnss.enableMsg(ublox::ZEDF9P0xB::CLASS_NAV, ublox::ZEDF9P0xB::NAV_PVT, true)) {
    cerr << "Failed to enable NAV_PVT message." << endl;
    return EXIT_FAILURE;
  }
  if (!gnss.enableMsg(ublox::ZEDF9P0xB::CLASS_NAV, ublox::ZEDF9P0xB::NAV_STATUS, true)) {
    cerr << "Failed to enable NAV_STATUS message." << endl;
    return EXIT_FAILURE;
  }
  if (!gnss.enableMsg(ublox::ZEDF9P0xB::CLASS_NAV, ublox::ZEDF9P0xB::NAV_TIMEGPS, true)) {
    cerr << "Failed to enable NAV_TIMEGPS message." << endl;
    return EXIT_FAILURE;
  }
  if (!gnss.enableMsg(ublox::ZEDF9P0xB::CLASS_NAV, ublox::ZEDF9P0xB::NAV_VELNED, true)) {
  ...
  while (true) {
    ...
    if (gnss.latestClass() != ublox::ZEDF9P0xB::CLASS_NAV) {
      continue;
    }

    switch (gnss.latestId()) {
      case ublox::ZEDF9P0xB::NAV_COV:
        cov.decode(gnss.payload());
        cout << "[NAV_COV]" << endl;
        cout << cov << endl;
        break;
      case ublox::ZEDF9P0xB::NAV_HPPOSLLH:
        hpposllh.decode(gnss.payload());
        cout << "[NAV_HPPOSLLH]" << endl;
        cout << hpposllh << endl;
        break;
      case ublox::ZEDF9P0xB::NAV_POSLLH:
        posllh.decode(gnss.payload());
        cout << "[NAV_POSLLH]" << endl;
        cout << posllh << endl;
        break;
      case ublox::ZEDF9P0xB::NAV_PVT:
        pvt.decode(gnss.payload());
        cout << "[NAV_PVT]" << endl;
        cout << pvt << endl;
        break;
      case ublox::ZEDF9P0xB::NAV_STATUS:
        status.decode(gnss.payload());
        cout << "[NAV_STATUS]" << endl;
        cout << status << endl;
        break;
      case ublox::ZEDF9P0xB::NAV_TIMEGPS:
        timegps.decode(gnss.payload());
        cout << "[NAV_TIMEGPS]" << endl;
        cout << timegps << endl;
        break;
      case ublox::ZEDF9P0xB::NAV_VELNED:
