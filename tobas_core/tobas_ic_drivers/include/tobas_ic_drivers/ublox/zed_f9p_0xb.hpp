...
{
/**
 * @brief A Linux driver of u-blox ZED-F9P-0xB using UART interface and UBX protocol.
 *
 * Product Page: https://www.u-blox.com/en/product/zed-f9p-module
 * Datasheet: https://content.u-blox.com/sites/default/files/documents/ZED-F9P-02B_DataSheet_UBX-21023276.pdf
...
{
private:
  static constexpr uint32_t kDefaultUartBaudRate = 38400;  // default baudrate
  static constexpr uint8_t kUartdataBits = 8;
  static constexpr size_t kUARTBufSize = 256;
  static constexpr uint8_t kRG174CableDelay = 5;  // [ns/m] 同軸ケーブルの遅延
  static constexpr double kWaitForGnssAck = 1.;   // [s]
  static constexpr auto kWaitForBaudRateChange = std::chrono::milliseconds(100); // uartのbaudrateを変更した際には100ms待てとかいてある

  // UARTで1バイト受け取る間隔 [us]
  // 小さいほど通信遅延を小さくできるが，小さすぎるとレシーバへのリクエスト過多で精度が落ちる．
  static constexpr auto kReqInterval = std::chrono::milliseconds(1);
...
public:
  ...
  bool configureBaudRate(uint32_t baudrate);
...
private:
  ...
  enum cfg_group_t : uint8_t
  {
    ...
    CFG_UART1 = 0x52,        // Configuration of the UART1 Interface
    ...
    CFG_UART2 = 0x53,        // Configuration of the UART2 Interface
  ...
  linux::UARTdev uart_;
  uint8_t tx_buf_[kUARTBufSize];
  uint8_t rx_buf_[kUARTBufSize];
...
inline ZEDF9P0xB::ubx_class_t ZEDF9P0xB::latestClass() const
{
  return static_cast<ubx_class_t>(*scanner_.getClass());
}

inline uint8_t ZEDF9P0xB::latestId() const
{
  return *scanner_.getId();
}

inline const uint8_t* ZEDF9P0xB::payload() const
{
