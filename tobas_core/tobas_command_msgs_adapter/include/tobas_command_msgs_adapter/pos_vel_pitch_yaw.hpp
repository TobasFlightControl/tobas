#pragma once

#include <rclcpp/type_adapter.hpp>

#include <tobas_kdl_msgs_adapter/vector.hpp>

#include <tobas_command_msgs/msg/pos_vel_pitch_yaw.hpp>

namespace tobas_command_msgs
{
struct PosVelPitchYaw
{
  std_msgs::msg::Header header;
  tobas_command_msgs::msg::Priority priority;
  kdl::Vector pos;
  kdl::Vector vel;
  double pitch;
  double yaw;

  using SharedPtr = std::shared_ptr<PosVelPitchYaw>;
  using ConstSharedPtr = std::shared_ptr<const PosVelPitchYaw>;
};
}  // namespace tobas_command_msgs

template <>
struct rclcpp::TypeAdapter<tobas_command_msgs::PosVelPitchYaw, tobas_command_msgs::msg::PosVelPitchYaw>
{
  using is_specialized = std::true_type;
  using custom_type = tobas_command_msgs::PosVelPitchYaw;
  using ros_message_type = tobas_command_msgs::msg::PosVelPitchYaw;

  static void convert_to_ros_message(const custom_type& src, ros_message_type& dst)
  {
    dst.header = src.header;
    dst.priority = src.priority;
    tobas_kdl_msgs::VectorAdapter::convert_to_ros_message(src.pos, dst.pos);
    tobas_kdl_msgs::VectorAdapter::convert_to_ros_message(src.vel, dst.vel);
    dst.pitch = src.pitch;
    dst.yaw = src.yaw;
  }

  static void convert_to_custom(const ros_message_type& src, custom_type& dst)
  {
    dst.header = src.header;
    dst.priority = src.priority;
    tobas_kdl_msgs::VectorAdapter::convert_to_custom(src.pos, dst.pos);
    tobas_kdl_msgs::VectorAdapter::convert_to_custom(src.vel, dst.vel);
    dst.pitch = src.pitch;
    dst.yaw = src.yaw;
  }
};

namespace tobas_command_msgs
{
using PosVelPitchYawAdapter =
  rclcpp::TypeAdapter<tobas_command_msgs::PosVelPitchYaw, tobas_command_msgs::msg::PosVelPitchYaw>;
}  // namespace tobas_command_msgs

RCLCPP_USING_CUSTOM_TYPE_AS_ROS_MESSAGE_TYPE(tobas_command_msgs::PosVelPitchYaw, tobas_command_msgs::msg::PosVelPitchYaw);
