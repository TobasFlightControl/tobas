...
private:
  a1::DShot dshot_;
  std::array<bool, a1::DShot::kChannelSize> is_enabled_;
...
DShotDriverNode::DShotDriverNode(const rclcpp::NodeOptions& options) : super("a1_dshot_driver", options)
{
...
void DShotDriverNode::throttlesCb(const tobas_msgs::msg::ThrottleArray::ConstSharedPtr& throttles)
{
  ...
  {
    if (elem.channel >= a1::DShot::kChannelSize)
    {
    ...
    {
      TOBAS_ASSERT(dshot_.setThrottle(elem.channel, a1::DShot::DISARM));
    }
    else
    {
      auto throttle = static_cast<uint16_t>(math::remap<double>(
        elem.throttle, tobas::kMinThrottle, tobas::kMaxThrottle, a1::DShot::kMinThrottle, a1::DShot::kMaxThrottle));
      throttle = clamp(throttle, a1::DShot::kMinThrottle, a1::DShot::kMaxThrottle);
...
{
  if (req->channel >= a1::DShot::kChannelSize)
  {
    res->success = false;
