 ...
 *
*/
// revised to support livox_lidar_sensor
...
namespace gazebo
{
  ...
  class RenderingSensorsPrivate;

  /// \class RenderingSensors RenderingSensors.hh gz/sim/systems/RenderingSensors.hh
  ...
  /// battery plugin charge reaches zero. RenderingSensors that are in nested
  /// models are also affected.
  ///
  /// \TODO(louise) Have one system for all sensors, or one per
  /// sensor / sensor type?
  class RenderingSensors:
    public System,
  ...
  {
    /// \brief Constructor
    public: explicit RenderingSensors();

    /// \brief Destructor
    public: ~RenderingSensors() override;
    ...
    private: std::unique_ptr<RenderingSensorsPrivate> dataPtr;
  };
...
class RenderingSensorsPrivate
{
  ...
  /// \brief RenderingSensors to include in the next rendering iteration
  public: std::set<sensors::SensorId> activeSensors;

  /// \brief RenderingSensors to be updated next
...
//////////////////////////////////////////////////
void RenderingSensorsPrivate::WaitForInit()
{
  while (!this->initialized && this->running)
  {
    ...
    {
      gzdbg << "this->doInit : " << this->doInit << std::endl;
      gzdbg << "this->running : " << this->running << std::endl;
    ...
    if (this->doInit)
    {
      ...
      gzdbg << "before renderUtil.Init()" << std::endl;
      this->renderUtil.Init();
      gzdbg << "after renderUtil.Init()" << std::endl;
#else
      // On macOS the render engine must be initialised on the main thread.
      // See RenderingSensors::Update.
#endif
      this->scene = this->renderUtil.Scene();
      this->scene->SetCameraPassCountPerGpuFlush(6u);
      gzdbg << "after SetCameraPssCoundPerGpuFlush" << std::endl;

      if (this->giVctParameters.enabled)
      {
        gzdbg << "in if this->giVctParameters.enabled" << std::endl;
      ...
      gzdbg << "before this->initialized = true" << std::endl;
      this->initialized = true;
    }

    gzdbg << "before this->updateAvailable = false" << std::endl;
...
//////////////////////////////////////////////////
void RenderingSensorsPrivate::RunOnce()
{
  {
  ...
  GZ_PROFILE("RenderingSensorsPrivate::RunOnce");
  {
    GZ_PROFILE("Update");
...
//////////////////////////////////////////////////
void RenderingSensorsPrivate::RenderThread()
{
  GZ_PROFILE_THREAD_NAME("RenderThread");

  gzdbg << "RenderingSensorsPrivate::RenderThread started" << std::endl;
  ...
  gzdbg << "RenderingSensorsPrivate::RenderThread stopped" << std::endl;
}

//////////////////////////////////////////////////
void RenderingSensorsPrivate::Run()
{
  gzdbg << "RenderingSensorsPrivate::Run" << std::endl;
  this->running = true;
  this->renderThread = std::thread(&RenderingSensorsPrivate::RenderThread, this);
}

//////////////////////////////////////////////////
void RenderingSensorsPrivate::Stop()
{
  gzdbg << "RenderingSensorsPrivate::Stop" << std::endl;
  std::unique_lock<std::mutex> lock(this->renderMutex);
  this->running = false;
...
//////////////////////////////////////////////////
void RenderingSensorsPrivate::ForceRender()
{
  this->forceUpdate = true;
}

//////////////////////////////////////////////////
void RenderingSensors::RemoveSensor(const Entity &_entity)
{
...
//////////////////////////////////////////////////
RenderingSensors::RenderingSensors() : System(), dataPtr(std::make_unique<RenderingSensorsPrivate>())
{
}

//////////////////////////////////////////////////
RenderingSensors::~RenderingSensors()
{
  this->dataPtr->Stop();
}
...
//////////////////////////////////////////////////
void RenderingSensors::Configure(const Entity &/*_id*/,
    const std::shared_ptr<const sdf::Element> &_sdf,
    EntityComponentManager &_ecm,
    EventManager &_eventMgr)
{
  gzdbg << "Configuring RenderingSensors system" << std::endl;
  ...
  this->dataPtr->renderUtil.SetEnableSensors(true,
      std::bind(&RenderingSensors::CreateSensor, this,
      std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
  this->dataPtr->renderUtil.SetRemoveSensorCb(
      std::bind(&RenderingSensors::RemoveSensor, this, std::placeholders::_1));
  ...
  this->dataPtr->stopConn = this->dataPtr->eventManager->Connect<events::Stop>(
      std::bind(&RenderingSensorsPrivate::Stop, this->dataPtr.get()));

  this->dataPtr->forceRenderConn = _eventMgr.Connect<events::ForceRender>(
      std::bind(&RenderingSensorsPrivate::ForceRender, this->dataPtr.get()));
...
//////////////////////////////////////////////////
void RenderingSensors::Reset(const UpdateInfo &_info, EntityComponentManager &)
{
  GZ_PROFILE("RenderingSensors::Reset");

  if (this->dataPtr->running && this->dataPtr->initialized)
  {
    gzdbg << "Resetting RenderingSensors\n";
...
//////////////////////////////////////////////////
void RenderingSensors::Update(const UpdateInfo &_info,
                     EntityComponentManager &_ecm)
{
  GZ_PROFILE("RenderingSensors::Update");
...
//////////////////////////////////////////////////
void RenderingSensors::PostUpdate(const UpdateInfo &_info,
                         const EntityComponentManager &_ecm)
{
  GZ_PROFILE("RenderingSensors::PostUpdate");
  // gzdbg << "in PostUpdatefunction" << std::endl;
  // gzdbg << "this->dataPtr->initialized : " << this->dataPtr->initialized << std::endl;
  // gzdbg << "this->dataPtr->forceUpdate : " << this->dataPtr->forceUpdate << std::endl;
  // gzdbg << "_ecm.HasComponentType(components::CustomSensor::typeId) : " << _ecm.HasComponentType(components::CustomSensor::typeId) << std::endl;
  {
    if (!this->dataPtr->initialized &&
        (this->dataPtr->forceUpdate ||
         ...
         _ecm.HasComponentType(components::SegmentationCamera::typeId) ||
         _ecm.HasComponentType(components::WideAngleCamera::typeId) ||
         _ecm.HasComponentType(components::CustomSensor::typeId)))
    {
      std::unique_lock<std::mutex> lock(this->dataPtr->renderMutex);
      gzdbg << "Initialization needed" << std::endl;
...
//////////////////////////////////////////////////
void RenderingSensorsPrivate::UpdateBatteryState(const EntityComponentManager &_ecm)
{
  // Battery state
...
//////////////////////////////////////////////////
std::chrono::steady_clock::duration RenderingSensorsPrivate::NextUpdateTime(
    std::set<sensors::SensorId> &_sensorsToUpdate,
    const std::chrono::steady_clock::duration &_currentTime)
{
...
//////////////////////////////////////////////////
bool RenderingSensorsPrivate::SensorsHaveConnections()
{
  for (auto id : this->sensorIds)
  {
    sensors::Sensor *s = this->sensorManager.Sensor(id);
...
RenderingSensorsPrivate::SensorsWithPendingTrigger()
{
...
GZ_ADD_PLUGIN(gazebo::RenderingSensors, System,
  gazebo::RenderingSensors::ISystemConfigure,
  gazebo::RenderingSensors::ISystemReset,
  gazebo::RenderingSensors::ISystemUpdate,
  gazebo::RenderingSensors::ISystemPostUpdate
)
