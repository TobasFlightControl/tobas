...
void WriteWPASupplicantThread::run()
{
  ...
  // 設定を書き込む
  if (ssh_client_.sftpWrite(kWPASupplicantPath, text_, true) != ssh::SSHClient::kNoError) {
    Q_EMIT finished(false, "SFTP-Write failed: " + QString(ssh_client_.errorMessage()));
    return;
  }

  // WiFiを再起動
  if (ssh_client_.execute("wpa_cli -i wlan0 reconfigure", true) != ssh::SSHClient::kNoError) {
