  ...
  double alpha)
{
  ...
  const Eigen::Vector3d translation = p.translation();
  const Ogre::Vector3 position(translation.x(), translation.y(), translation.z());
  ...
  switch (s->type) {
    case shapes::SPHERE: {
      ...
      const auto d = 2. * static_cast<const shapes::Sphere*>(s)->radius;
    ...
    case shapes::CYLINDER: {
      ...
      const auto d = 2. * static_cast<const shapes::Cylinder*>(s)->radius;
      const auto z = static_cast<const shapes::Cylinder*>(s)->length;
      // The shape has z as major axis, but the rendered cylinder has y as major axis (assuming z is upright).
      ogre_shape->setScale(Ogre::Vector3(d, z, d));
      break;
    }
    case shapes::MESH: {
      const auto mesh = static_cast<const shapes::Mesh*>(s);
      if (mesh->triangle_count > 0) {
        const auto m = new rviz_rendering::MeshShape(context_->getSceneManager(), node);
        ogre_shape = m;
        ...
        for (uint32_t i = 0; i < mesh->triangle_count; ++i) {
          ...
          for (int k = 0; k < 3; ++k) {
            const auto vi = 3 * mesh->triangles[i3 + k];
            const Ogre::Vector3 v(mesh->vertices[vi], mesh->vertices[vi + 1], mesh->vertices[vi + 2]);
            if (mesh->vertex_normals) {
              const Ogre::Vector3 n(
                mesh->vertex_normals[vi], mesh->vertex_normals[vi + 1], mesh->vertex_normals[vi + 2]);
              m->addVertex(v, n);
            }
  ...
  if (ogre_shape) {
    ...
    if (s->type == shapes::CYLINDER) {
      ...
      // For the rviz shape, the y axis is the height.
      // We add a transform to fix this.
      static Ogre::Quaternion fix(Ogre::Radian(M_PI_2), Ogre::Vector3::UNIT_X);
...
{
  for (const auto& shape : scene_shapes_) {
