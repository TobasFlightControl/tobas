...
{
  return tobas::getLogger("tobas.joint_model_group");
}
}  // namespace
...
bool JointModelGroup::satisfiesPositionBounds(
  const double* state,
  const JointBoundsVector& active_joint_bounds,
  double margin) const
{
...
bool JointModelGroup::computeJointVariableIndices(
...
void JointModelGroup::setSolverAllocators(const std::pair<SolverAllocatorFn, SolverAllocatorMapFn>& solvers)
{
  if (solvers.first) {
    ...
    if (group_kinematics_.first.solver_instance_) {
      ...
      if (!computeJointVariableIndices(
            group_kinematics_.first.solver_instance_->getJointNames(), group_kinematics_.first.bijection_)) {
...
{
  const KinematicsBaseConstPtr& solver = getSolverInstance();
  if (!solver || tip.empty()) {
    return false;
  }
  ...
  for (const std::string& tip_frame : tip_frames) {
    ...
    RCLCPP_DEBUG(
      getLogger(), "comparing input tip: %s to this groups tip: %s ", tip_local.c_str(), tip_frame_local.c_str());
...
std::pair<Eigen::VectorXd, Eigen::VectorXd> JointModelGroup::getLowerAndUpperLimits() const
{
  ...
  for (const JointModel::Bounds* joint_bounds : active_joint_models_bounds_) {
    for (const VariableBounds& variable_bounds : *joint_bounds) {
...
std::pair<Eigen::VectorXd, Eigen::VectorXd> JointModelGroup::getMaxVelocitiesAndAccelerationBounds() const
{
  ...
  for (std::size_t i = 0; i < active_joint_models_bounds_.size(); ++i) {
    ...
    max_joint_accelerations[i] = std::min(
      -active_joint_models_bounds_[i]->at(0).min_acceleration_, active_joint_models_bounds_[i]->at(0).max_acceleration_);
  }
  return { max_joint_velocities, max_joint_accelerations };
}
}  // namespace tobas
