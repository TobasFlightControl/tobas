...
void RobotState::setToRandomPositionsNearBy(
  ...
  random_numbers::RandomNumberGenerator& rng)
{
  ...
  for (const JointModel* joint : joints)
  {
    ...
    joint->getVariableRandomPositionsNearBy(
      rng, &position_.at(joint->getFirstVariableIndex()), &seed.position_.at(idx), distance);
  }
  updateMimicJoints(group);
}
...
const LinkModel*
RobotState::getRigidlyConnectedParentLinkModel(const std::string& frame, const JointModelGroup* jmg) const
{
...
void RobotState::interpolate(const RobotState& to, double t, RobotState& state, const JointModelGroup* joint_group)
  const
{
  checkInterpolationParamBounds(getLogger(), t);
  const std::vector<const JointModel*>& jm = joint_group->getActiveJointModels();
...
{
  attachBody(std::make_unique<AttachedBody>(
    robot_model_->getLinkModel(link), id, pose, shapes, shape_poses, touch_links, detach_posture, subframe_poses));
}
...
void RobotState::getAttachedBodies(std::vector<const AttachedBody*>& attached_bodies, const JointModelGroup* group)
  const
{
  attached_bodies.clear();
  for (const auto& it : attached_body_map_)
...
Eigen::MatrixXd
RobotState::getJacobian(const JointModelGroup* group, const Eigen::Vector3d& reference_point_position) const
{
  ...
  bool use_quaternion_representation) const
{
  ...
  // displacement at the tip. So we build the Jacobian incrementally joint by joint up to the parent joint of the
  // reference link.
...
void RobotState::computeVariableVelocity(
  const JointModelGroup* jmg,
  Eigen::VectorXd& qdot,
  const Eigen::VectorXd& twist,
...
bool RobotState::integrateVariableVelocity(
  const JointModelGroup* jmg,
...
{
  const KinematicsBaseConstPtr& solver = jmg->getSolverInstance();
  if (!solver)
  {
...
bool ikCallbackFnAdapter(
  ...
  const JointModelGroup* group,
  const GroupStateValidityCallbackFn& constraint,
  const geometry_msgs::msg::Pose& /*unused*/,
  ...
  tobas_visualization_msgs::msg::ErrorCodes& error_code)
{
  ...
  {
    error_code.val = tobas_visualization_msgs::msg::ErrorCodes::SUCCESS;
  }
  else
  {
    error_code.val = tobas_visualization_msgs::msg::ErrorCodes::NO_IK_SOLUTION;
  }
  return true;
}
}  // namespace

bool RobotState::setToIKSolverFrame(Eigen::Isometry3d& pose, const KinematicsBaseConstPtr& solver)
{
...
bool RobotState::setFromIK(
  ...
  const Eigen::Isometry3d& pose_in,
  const std::string& tip_in,
  const std::vector<double>& consistency_limits_in,
  double timeout,
  const GroupStateValidityCallbackFn& constraint,
  const KinematicsQueryOptions& options,
  const KinematicsBase::IKCostFn& cost_function)
  ...
  const KinematicsBase::IKCostFn& cost_function)
{
  ...
  tobas_visualization_msgs::msg::ErrorCodes error;
...
bool RobotState::setFromIKSubgroups(
  const JointModelGroup* jmg,
  const EigenSTL::vector_Isometry3d& poses_in,
  const std::vector<std::string>& tips_in,
  const std::vector<std::vector<double> >& consistency_limits,
...
void RobotState::getStateTreeJointString(std::ostream& ss, const JointModel* jm, const std::string& pfx0, bool last)
  const
{
  std::string pfx = pfx0 + "+--";
