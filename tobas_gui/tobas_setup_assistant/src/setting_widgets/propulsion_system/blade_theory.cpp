...
#include "tobas_setup_assistant/setting_tabs/propulsion_system/blade_theory.hpp"
  ...
  : N_(num_blades), R_(radius), c_(blade_chord), theta_(pitch_angle), rho_(air_density)
{
  ...
  TOBAS_CHECK(0. < pitch_angle && pitch_angle < M_PI);
  TOBAS_CHECK(air_density > 0.);
}

double BladeTheory::motorConst() const
{
  return 4 * M_PI * C_T() * rho_ * math::quat(R_);
}
