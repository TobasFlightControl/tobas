...
void MeasureSensorNoise::run()
{
  ...
  for (size_t i = 0; i < kDataCount; ++i)
  {
    ...
    usleep(tobas_navio_ros::kWaitToRefreshBarometer);
  ...
  pt.put(tobas_navio_ros::kConfigKey_AccNoiseDensity, acc_noise_density);
  pt.put(tobas_navio_ros::kConfigKey_GyroNoiseDensity, gyro_noise_density);
  pt.put(tobas_navio_ros::kConfigKey_MagNoiseDensity, mag_noise_density);
  pt.put(tobas_navio_ros::kConfigKey_PressureNoiseDensity, pres_noise_density);
  pt.save();
  cout << "The result is saved to '" << tobas_navio_ros::kConfigPath << "'." << endl;
...
void MeasureSensorNoise::sendDisarm()
{
  ...
  {
    setPeriodOnAllChannels(tobas_navio_ros::kPwmDisarm);
    usleep(tobas_navio_ros::kDisarmInterval * 1000000);
  }
}
