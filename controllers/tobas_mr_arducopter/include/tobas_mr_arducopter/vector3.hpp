...
public:
  ...
  // setting ctor
  constexpr Vector3<T>(const T x0, const T y0, const T z0) : x(x0), y(y0), z(z0)
  {
  }

  // Create a Vector3 from a Vector2 with z
  constexpr Vector3<T>(const Vector2<T>& v0, const T z0) : x(v0.x), y(v0.y), z(z0)
  {
  }
  ...
  // distance from the tip of this vector to another vector squared (so as to avoid the sqrt
  // calculation)
  ...
  static Vector3<T> point_on_line_closest_to_other_point(
    const Vector3<T>& w1,
    const Vector3<T>& w2,
    const Vector3<T>& p);
  ...
  static void segment_to_segment_closest_point(
    const Vector3<T>& seg1_start,
    const Vector3<T>& seg1_end,
    const Vector3<T>& seg2_start,
    const Vector3<T>& seg2_end,
    Vector3<T>& closest_point);

  // Returns true if the passed 3D segment passes through a plane defined by plane normal, and a
  // point on the plane
  static bool segment_plane_intersect(
    const Vector3<T>& seg_start,
    const Vector3<T>& seg_end,
    const Vector3<T>& plane_normal,
    const Vector3<T>& plane_point);
};

// check if all elements are zero
template <>
inline bool Vector3<float>::is_zero(void) const
{
  return x == 0 && y == 0 && z == 0;
}

template <>
inline bool Vector3<double>::is_zero(void) const
{
  return x == 0 && y == 0 && z == 0;
}
...
Matrix3<T> Vector3<T>::mul_rowcol(const Vector3<T>& v2) const
{
  ...
  return Matrix3<T>(
    v1.x * v2.x, v1.x * v2.y, v1.x * v2.z, v1.y * v2.x, v1.y * v2.y, v1.y * v2.z, v1.z * v2.x,
    v1.z * v2.y, v1.z * v2.z);
}
...
T Vector3<T>::distance_to_segment(const Vector3<T>& seg_start, const Vector3<T>& seg_end) const
{
  ...
  // area must be constrained above 0 because a triangle could have 3 points could be on a line and
  // float rounding could push this under 0
...
// Shortest distance between point(p) to a point contained in the line segment defined by w1,w2
template <typename T>
T Vector3<T>::closest_distance_between_line_and_point(
  const Vector3<T>& w1,
  const Vector3<T>& w2,
  const Vector3<T>& p)
{
...
template <typename T>
Vector3<T> Vector3<T>::point_on_line_closest_to_other_point(
  const Vector3<T>& w1,
  const Vector3<T>& w2,
  const Vector3<T>& p)
{
...
// OUTPUT: closest point on segment 2, from segment 1, gets passed on reference as "closest_point"
template <typename T>
void Vector3<T>::segment_to_segment_closest_point(
  const Vector3<T>& seg1_start,
  const Vector3<T>& seg1_end,
  const Vector3<T>& seg2_start,
  const Vector3<T>& seg2_end,
...
// Returns true if the passed 3D segment passes through a plane defined by plane normal, and a point
// on the plane
template <typename T>
bool Vector3<T>::segment_plane_intersect(
  const Vector3<T>& seg_start,
  const Vector3<T>& seg_end,
  const Vector3<T>& plane_normal,
  const Vector3<T>& plane_point)
