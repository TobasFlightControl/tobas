...
#define HALF_SQRT_HALF_TIMES_TWO_PLUS_SQRT_TWO                                                     \
  0.65328148243818828788676000840496  // sqrt((2 + sqrt(2))/2) / 2
#define HALF_SQRT_HALF_TIMES_TWO_MINUS_SQRT_TWO                                                    \
  0.27059805007309845059637609665515  // sqrt((2 - sqrt(2))/2) / 2
...
public:
  ...
  // setting constructor
  QuaternionT(const T _q[4]) : q1(_q[0]), q2(_q[1]), q3(_q[2]), q4(_q[3])
  {
  }
  ...
  // absolute (e.g. always positive) earth-frame roll-pitch difference (in radians) between this
