...
const AP_Param::GroupInfo AC_AttitudeControl_Multi::var_info[] = {
  ...
  // @Description: Roll axis rate controller P gain.  Corrects in proportion to the difference
  ...
  // @Description: Roll axis rate controller I gain.  Corrects long-term difference in desired roll
  ...
  // @Description: Roll axis rate controller I gain maximum.  Constrains the maximum that the I term
  // will output
  // @Range: 0 1
  // @Increment: 0.01
  // @User: Standard
  ...
  // @Description: Roll axis rate controller D gain.  Compensates for short-term change in desired
  ...
  // @Description: Sets an upper limit on the slew rate produced by the combined P and D gains. If
  // the amplitude of the control action produced by the rate feedback exceeds this value, then the
  // D+P gain is reduced to respect the limit. This limits the amplitude of high frequency
  // oscillations caused by an excessive gain. The limit should be set to no more than 25% of the
  // actuators maximum slew rate to allow for load effects. Note: The gain will not be reduced to
  // less than 10% of the nominal value. A value of zero will disable this feature.
  // @Range: 0 200
  // @Increment: 0.5
  // @User: Advanced
  ...
  // @Description: Pitch axis rate controller P gain.  Corrects in proportion to the difference
  ...
  // @Description: Pitch axis rate controller I gain.  Corrects long-term difference in desired
  ...
  // @Description: Pitch axis rate controller I gain maximum.  Constrains the maximum that the I
  // term will output
  // @Range: 0 1
  // @Increment: 0.01
  // @User: Standard
  ...
  // @Description: Pitch axis rate controller D gain.  Compensates for short-term change in desired
  // pitch rate vs actual pitch rate
  // @Range: 0.0 0.05
  // @Increment: 0.001
  // @User: Standard
  ...
  // @Description: Yaw axis rate controller P gain.  Corrects in proportion to the difference
  ...
  // @Description: Yaw axis rate controller I gain.  Corrects long-term difference in desired yaw
  ...
  // @Description: Yaw axis rate controller I gain maximum.  Constrains the maximum that the I term
  // will output
  // @Range: 0 1
  // @Increment: 0.01
  // @User: Standard
  ...
  // @Description: Yaw axis rate controller D gain.  Compensates for short-term change in desired
  ...
  // @Description: Throttle vs attitude control prioritisation used when landing (higher values mean
  // we prioritise attitude control over throttle)
  // @Range: 0.1 0.25
  // @User: Advanced
  AP_GROUPINFO(
    "THR_MIX_MIN",
  ...
  // @Description: Throttle vs attitude control prioritisation used during active flight (higher
  ...
  // @Description: Throttle vs attitude control prioritisation used during manual flight (higher
  // values mean we prioritise attitude control over throttle)
  // @Range: 0.1 0.9
  // @User: Advanced
  AP_GROUPINFO(
  ...
  // @Description: Throttle-gain boost ratio. A value of 0 means no boosting is applied, a value of
  // 1 means full boosting is applied. Describes the ratio increase that is applied to angle P and
  // PD on pitch and roll.
  // @Range: 0 1
  // @User: Advanced
...
void AC_AttitudeControl_Multi::update_althold_lean_angle_max(float throttle_in)
{
  ...
  float althold_lean_angle_max = acosf(constrain_float(
    throttle_in / (AC_ATTITUDE_CONTROL_ANGLE_LIMIT_THROTTLE_MAX * thr_max), 0.0f, 1.0f));
  _althold_lean_angle_max =
    _althold_lean_angle_max
    + (_dt / (_dt + _angle_limit_tc)) * (althold_lean_angle_max - _althold_lean_angle_max);
}

void AC_AttitudeControl_Multi::set_throttle_out(
  float throttle_in,
...
float AC_AttitudeControl_Multi::get_throttle_avg_max(float throttle_in)
{
  ...
  return MAX(
    throttle_in, throttle_in * MAX(0.0f, 1.0f - _throttle_rpy_mix)
                   + _motors.get_throttle_hover() * _throttle_rpy_mix);
}
...
void AC_AttitudeControl_Multi::update_throttle_gain_boost()
{
  ...
  if (_motors.get_throttle_slew_rate() > AC_ATTITUDE_CONTROL_THR_G_BOOST_THRESH)
  {
    ...
    const float angle_p_boost =
      constrain_float((_throttle_gain_boost + 1.0f) * (_throttle_gain_boost + 1.0f), 1.0, 4.0);
...
void AC_AttitudeControl_Multi::update_throttle_rpy_mix()
{
  ...
  else if (_throttle_rpy_mix > _throttle_rpy_mix_desired)
  {
    ...
    float mix_used;
    // since throttle_out >= throttle_in at this point we don't need to check throttle_in <
    // throttle_hover
...
void AC_AttitudeControl_Multi::rate_controller_run()
{
  ...
  // move throttle vs attitude mixing towards desired (called from here because this is conveniently
  ...
  _motors.set_roll(
    get_rate_roll_pid().update_all(
      _ang_vel_body.x, gyro_latest.x, _dt, _motors.limit.roll, _pd_scale.x)
    + _actuator_sysid.x);
  _motors.set_roll_ff(get_rate_roll_pid().get_ff());

  _motors.set_pitch(
    get_rate_pitch_pid().update_all(
      _ang_vel_body.y, gyro_latest.y, _dt, _motors.limit.pitch, _pd_scale.y)
    + _actuator_sysid.y);
  _motors.set_pitch_ff(get_rate_pitch_pid().get_ff());

  _motors.set_yaw(
    get_rate_yaw_pid().update_all(
      _ang_vel_body.z, gyro_latest.z, _dt, _motors.limit.yaw, _pd_scale.z)
    + _actuator_sysid.z);
