...
void PoseTwistAccelController::initialize(tobas::BaseNode* node)
{
  ...
  cmd_pub_ = node->createPublisher<tobas_command_msgs::PoseTwistAccel>(tobas::kPoseTwistAccelCmdTopic);
}
...
void PoseTwistAccelController::update(const tobas_msgs::msg::RCInput& rcin, const tobas_msgs::Odometry& odom)
{
  ...
  const auto tar_vel_W = kdl::Rotation::RotZ(tar_rpy_.yaw) * tar_vel_G_;
  ...
  if (!is_up_commanded_)
  {
    ...
    tar_rpy_.yaw = odom.frame.M.getYaw();
