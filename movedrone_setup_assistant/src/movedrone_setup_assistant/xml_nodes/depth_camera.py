...
def add_depth_camera_model(
    ...
    optical_center_link = f'{ns}/depth_camera_optical_center_link'

    # robot/link
    robot.append(DummyLink(depth_camera_link))
    robot.append(DummyLink(optical_center_link))
    ...
    robot.append(Joint(
        name=f'{ns}/depth_camera_optical_center_joint',
        type="fixed",
        origin=Origin(0., 0., 0., -math.pi / 2, 0., -math.pi / 2),
        parent=Parent(depth_camera_link),
        child=Child(optical_center_link),
    ))
    ...
    ET.SubElement(plugin, "frameName").text = optical_center_link
    ET.SubElement(plugin, "updateRate").text = f'{frame_rate}'
    ET.SubElement(plugin, "depthNoiseModel").text = noise_model
    ET.SubElement(plugin, "horizontalFOV").text = f'{horizontal_fov}'
