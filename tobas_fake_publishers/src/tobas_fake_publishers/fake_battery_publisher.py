...
class FakeBatteryPublisher:
    ...
    def __init__(self) -> None:
        ...
        voltage = rospy.get_param("~voltage", self.DEFAULT_VOLTAGE)
        current = rospy.get_param("~current", self.DEFAULT_CURRENT)

        self._battery_msg = Battery()
        self._battery_msg.voltage = voltage
        self._battery_msg.current = current

        self._battery_pub = rospy.Publisher(Topic.BATTERY, Battery, queue_size=1)
