  ...
  : nh_(nh), nh_private_(nh_private), initialized_filter_(false)
{
  ...
  // Register publishers:
  imu_publisher_ =
    nh_.advertise<sensor_msgs::Imu>(ros::names::resolve("imu") + "/data", queue_size);
  ...
  if (publish_debug_topics_)
  {
    ...
    {
      state_publisher_ =
        nh_.advertise<std_msgs::Bool>(ros::names::resolve("imu") + "/steady_state", queue_size);
    }
  }

  // Register IMU raw data subscriber.
  imu_subscriber_.reset(
    new ImuSubscriber(nh_, ros::names::resolve("imu") + "/data_raw", queue_size));
...
void ComplementaryFilterROS::initializeParams()
{
  ...
  nh_private_.param("time_jump_threshold", time_jump_threshold, time_jump_threshold);
...
tf::Quaternion
ComplementaryFilterROS::hamiltonToTFQuaternion(double q0, double q1, double q2, double q3) const
{
  ...
  if (publish_tf_)
  {
    ...
    if (reverse_tf_)
    {
      tf_broadcaster_.sendTransform(tf::StampedTransform(
        transform.inverse(), imu_msg_raw->header.stamp, imu_msg_raw->header.frame_id,
        fixed_frame_));
    }
    else
    {
      tf_broadcaster_.sendTransform(tf::StampedTransform(
        transform, imu_msg_raw->header.stamp, fixed_frame_, imu_msg_raw->header.frame_id));
    }
  }
}
...
void ComplementaryFilterROS::checkTimeJump()
{
  ...
  ROS_WARN(
    "Detected jump back in time of %.1f s. Resetting IMU filter.", (last_ros_time_ - now).toSec());
